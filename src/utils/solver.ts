...
export function animateFace(meshes: any[], blendshapes: any[]) {
...
export function rotateHead(bones: any[], faceMatrix: number[]) {
...
export function animateBody(
  ...
  const shoulderY = poseLms[lSHOULDER]
    .clone()
    .lerp(poseLms[rSHOULDER], 0.5)
    .normalize();
...
export function animateHand(bone: any, landmarks: any[], handedness: string) {
  // cache landmarks
  landmarks.forEach((landmark, lmIdx) => {
...
export function resetBlendshapes(meshes: any[]) {
  for (const mesh of meshes) {
...
export function resetRotations(bones: any[], quats: any[]) {
...
// z: along spine
function createShoulderAxes(landmarkFrom: any, landmarkTo: any) {
...
function createHipAxes(lHipLm: any, rHipLm: any) {
...
function solveHand(avatarWristBone: any, handedness: string) {
...
function clampRadiansToDegrees(value: number, min: number, max: number) {
